<div align="center">
  ...
  **本地优先的 MySQL / Redis 工作台：编辑数据与结构，让 AI 受控访问数据库，并离线还原 Binlog。**
...
Pipa 是面向 macOS 的本地优先数据库工作台，覆盖 MySQL 与 Redis 日常操作、MySQL 数据与结构编辑、本机模型上下文协议（Model Context Protocol，MCP）接入和离线 Binlog 分析。

应用不依赖 Pipa 自建云端服务。它只会访问用户明确配置的数据库、本机回环地址上的 MCP 服务，以及用于检查和下载软件更新的 GitHub Releases；Monaco 编辑器等运行时代码均随应用本地打包。

## 用 Pipa 完成这些工作

Pipa 将数据库浏览、编辑、自动化接入和事务取证放在同一个桌面工作区中：

| 场景 | 能力 |
| --- | --- |
| SQL 查询 | Monaco 编辑器、选中或光标语句执行、取消查询、耗时与结果状态、常用 SQL 库 |
| 数据与结构 | 分页浏览、批量暂存编辑、原子提交、可视化表结构、索引与表操作 |
| Redis | 数据库与键浏览、常见数据类型创建与编辑、TTL、重命名、原生命令工作区 |
| AI 接入 | 仅监听本机回环地址的 MCP 服务、连接范围控制、只读直通、写入审批 |
| Binlog | 多文件离线解析、事务与行镜像查看、完整性诊断、Reset SQL 预览 |

## MCP：AI 可读，写入需确认

Pipa 内置仅监听 `127.0.0.1` 的 Streamable HTTP MCP 服务，让本机 AI 工具读取已授权的 MySQL 连接、表结构和数据，也能直接分析 Binlog。

- 只读 SQL 通过安全策略后执行，最多返回 200 行；写入和结构变更语句只进入待确认队列。
- Bearer Token、可多选的连接作用域和本机回环监听共同限制访问范围。
- MCP 控制台优先显示待审批 SQL，并提供运行状态、活动记录和无限制作用域提醒。
- 打开 MCP 控制台即可复制 URL、Token 或 Cursor 配置；Token 默认遮罩并支持轮换。
...
- 快捷搜索支持按连接信息过滤；连接树中的 MySQL 表、Redis 数据库和键均可单击打开。
- 侧边栏宽度及 SQL 编辑器与结果区高度可拖动调整，并在本机持久化。
- 标签支持拖拽排序、右键批量操作和独立窗口；`Cmd/Ctrl+1–9` 可按位置跳转，数字 9 始终打开最后一个标签。
- 执行中的 SQL 标签保持关闭保护，同时允许切换到其他工作区继续操作。

### MySQL 与 SQL 工作区

- 创建、测试、重命名和整理连接，在独立标签中编写与执行 SQL。
- 执行选中 SQL 或光标所在语句；状态栏持续显示等待时间、耗时、返回或影响行数，并可取消运行中的查询。
- 流式展示查询结果，保留 JSON、大整数、精确小数、二进制和时间值的原始语义；数据库文本与 SQL 预览保持可读字符串。
- 结果区支持连续区域和不连续整行选择、搜索、排序、列宽调整，以及 CSV、TSV、JSON、Markdown、SQL INSERT 与 IN 列表导出。
- 在数据表中暂存新增、修改和删除，经过类型校验后使用参数化语句在一个事务中提交；冲突或失败会整体回滚。
- 可视化编辑字段类型、长度、无符号属性、字符集、排序规则、注释和索引，并在执行前检查生成的结构变更 SQL。
- 表操作菜单支持复制名称、重命名、复制表、清空、删除、置顶、独立窗口、数据导出和查看 `CREATE TABLE`。
- 在工作区之间切换时保留本次运行内的编辑器与查询结果状态；重启后恢复未保存的 SQL 与标签上下文，但不会永久保存查询结果。

### Redis 工作区

- 创建、测试和整理 Redis 连接，单击切换 DB 或打开键。
- 查看并按类型创建或编辑 String、Hash、List、Set、Sorted Set、Stream 与 RedisJSON 等键。
- 设置或移除 TTL、重命名和删除键；写操作会先显示待执行命令，生产连接额外要求确认。
- 在命令工作区执行 Redis 原生命令。
...
| 数据库 | 当前支持 |
| --- | --- |
| MySQL | 连接管理、SQL 查询、数据与结构编辑、表操作、离线 Binlog 分析与 MCP 访问 |
| Redis | 连接管理、数据库切换、键浏览与编辑、TTL、重命名与原生命令执行 |
